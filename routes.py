...
def index():
    ...
    habits_on_date = [e for e in current_app.db.habits.find(
...
@ pages.route("/dodaj", methods=["GET", "POST"])
def add_habit():
...
def complete():
    ...
    current_app.db.habits.update_one({"_id": habit},
                                     {"$set": {"cpl_date": date}})
